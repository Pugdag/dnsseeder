...
[![GoDoc](https://img.shields.io/badge/godoc-reference-blue.svg)](http://godoc.org/github.com/Pugdag/dnsseeder)

DNSSeeder exposes a list of known peers to any new peer joining the
Pugdagnetwork via the DNS protocol.

When DNSSeeder is started for the first time, it will connect to the
`pugdagd` node specified with the `-s` flag and listen for `addr`
...
file, so on subsequent start ups the `pugdagd` node specified with `-s`
...
* Launch a `pugdagd` node for the DNSSeeder to connect to
...
```bash
git clone https://github.com/Pugdag/dnsseeder ${GOPATH}/src/github.com/Pugdag/dnsseeder
cd ${GOPATH}/src/github.com/Pugdag/dnsseeder
go install .
```
...
`pugdagd` instances, set the following DNS records:
